...
public class PredictorDecompress3950toCurrent extends IPredictorDecompress {
  ...
  public int DecompressValue(int nA, int nB) {
    ...
    int ai2[];
    int ai4[];
    int nPredictionA = (l * (ai2 = m_aryMA)[0]) + (ai[l1] * ai2[1]) + (ai[indexA - 2] * ai2[2])
        + (ai[indexA - 3] * ai2[3]);
    int nPredictionB = (ai3[indexB] * (ai4 = m_aryMB)[0]) + (ai3[k2] * ai4[1])
        + (ai3[indexB - 2] * ai4[2]) + (ai3[indexB - 3] * ai4[3]) + (ai3[indexB - 4] * ai4[4]);
  ...
  // protected ScaledFirstOrderFilter m_Stage1FilterA = new
  // ScaledFirstOrderFilter(31, 5);
  protected int scaledFilterALV;
  // protected ScaledFirstOrderFilter m_Stage1FilterB = new
  // ScaledFirstOrderFilter(31, 5);
