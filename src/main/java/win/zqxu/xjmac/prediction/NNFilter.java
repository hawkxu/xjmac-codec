...
import win.zqxu.xjmac.tools.RollBufferShort;
...
public abstract class NNFilter {
  ...
  public int Compress(int nInput) {
    ...
    inputData[inputIndex] = (short) ((nInput >= Short.MIN_VALUE && nInput <= Short.MAX_VALUE)
        ? nInput
        : (nInput >> 31) ^ 0x7FFF);
  ...
  public int Decompress(int nInput) {
    ...
    inputData[inputIndex] = (short) ((nOutput >= Short.MIN_VALUE && nOutput <= Short.MAX_VALUE)
        ? nOutput
        : (nOutput >> 31) ^ 0x7FFF);
  ...
  protected abstract void AdaptNoMMX(short[] pM, int indexM, short[] pAdapt, int indexA,
      int nDirection);
}
