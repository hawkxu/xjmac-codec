 ...
 *----------------------------------------------------------------------
 */
package win.zqxu.xjmac.core;

import java.io.IOException;
import java.io.RandomAccessFile;

import win.zqxu.xjmac.decoder.IAPEDecompress;
import win.zqxu.xjmac.encoder.IAPECompress;
import win.zqxu.xjmac.info.APEFileInfo;
import win.zqxu.xjmac.info.InputSource;
import win.zqxu.xjmac.info.WaveFormat;
import win.zqxu.xjmac.tools.*;

/**
 * Author: Dmitry Vaguine Date: 11.05.2004 Time: 16:26:19
 */
public class APESimple {
  ...
  public static void VerifyFile(String pInputFilename, ProgressCallback progressor)
      throws IOException, JMACSkippedException, JMACStoppedByUserException {
    ...
    try {
      ...
      if ((pInfo.nVersion < 3980) || (pInfo.spAPEDescriptor == null))
        ...
        int nStart = (int) (nHead + pInfo.spAPEDescriptor.nHeaderBytes
            + pInfo.spAPEDescriptor.nSeekTableBytes);
        ...
        int nBytesLeft = (int) (pInfo.spAPEDescriptor.nHeaderDataBytes
            + pInfo.spAPEDescriptor.nAPEFrameDataBytes
            + pInfo.spAPEDescriptor.nTerminatingDataBytes);
   ...
   * ***************************************************************************************
   */
  public static void ConvertFile(String pInputFilename, String pOutputFilename,
      int nCompressionLevel, ProgressCallback progressor)
      throws IOException, JMACSkippedException, JMACStoppedByUserException {
    DecompressCore(pInputFilename, pOutputFilename, UNMAC_DECODER_OUTPUT_APE, nCompressionLevel,
        progressor);
  }
   ...
   * ***************************************************************************************
   */
  public static void DecompressFile(String pInputFilename, String pOutputFilename,
      ProgressCallback progressor)
      throws IOException, JMACSkippedException, JMACStoppedByUserException {
  ...
  public static void DecompressCore(String pInputFilename, String pOutputFilename, int nOutputMode,
      int nCompressionLevel, ProgressCallback progressor)
      throws IOException, JMACSkippedException, JMACStoppedByUserException {
    // error check the function parameters
    if (pInputFilename == null)
  ...
  public static void DecompressCore(IAPEDecompress spAPEDecompress, String pOutputFilename,
      int nOutputMode, int nCompressionLevel, ProgressCallback progressor)
      throws IOException, JMACSkippedException, JMACStoppedByUserException {
    // variable declares
    ...
    try {
      ...
      byte[] waveHeaderBuffer = spAPEDecompress
          .getApeInfoWavHeaderData(spAPEDecompress.getApeInfoWavHeaderBytes());
      ...
      if (nOutputMode == UNMAC_DECODER_OUTPUT_WAV) {
        ...
        if (spAPEDecompress.getApeInfoFileVersion() == Globals.MAC_VERSION_NUMBER
            && spAPEDecompress.getApeInfoCompressionLevel() == nCompressionLevel)
          throw new JMACSkippedException();

        // create and start the compressor
        spAPECompress = IAPECompress.CreateIAPECompress();
        spAPECompress.Start(pOutputFilename, wfeInput,
            spAPEDecompress.getApeInfoDecompressTotalBlocks()
                * spAPEDecompress.getApeInfoBlockAlign(),
      ...
      // create the progress helper
      ProgressHelper spMACProgressHelper = new ProgressHelper(nBlocksLeft / BLOCKS_PER_DECODE,
          progressor);
      ...
      while (nBlocksLeft > 0) {
        ...
        if (nOutputMode == UNMAC_DECODER_OUTPUT_WAV)
          ...
          spAPECompress.AddData(spTempBuffer,
              nBlocksDecoded * spAPEDecompress.getApeInfoBlockAlign());
      ...
      if (nOutputMode == UNMAC_DECODER_OUTPUT_WAV) {
        ...
        if (spAPEDecompress.getApeInfoWavTerminatingBytes() > 0) {
          byte[] termData = spAPEDecompress
              .getApeInfoWavTerminatingData(spAPEDecompress.getApeInfoWavTerminatingBytes());
        ...
        if (nTerminatingBytes > 0) {
          ...
          if (bHasTag) {
            spAPEDecompress.getApeInfoIoSource()
                .seek(spAPEDecompress.getApeInfoIoSource().length() - nTagBytes);
            spAPEDecompress.getApeInfoIoSource().read(spTempBuffer,
                spAPEDecompress.getApeInfoWavTerminatingBytes(), nTagBytes);
          }

          spAPECompress.Finish(spTempBuffer, nTerminatingBytes,
              spAPEDecompress.getApeInfoWavTerminatingBytes());
        } else
  ...
  public static void CompressFile(String pInputFilename, String pOutputFilename,
      int nCompressionLevel, ProgressCallback progressor)
      throws IOException, JMACStoppedByUserException {
