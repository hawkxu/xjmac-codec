...
public class APEDecompressOld extends IAPEDecompress {
  ...
  public APEDecompressOld(APEInfo pAPEInfo, int nStartBlock, int nFinishBlock) {
    ...
    // version check (this implementation only works with 3.92 and earlier
    // files)
    ...
    m_nFinishBlock = (nFinishBlock < 0) ? getApeInfoTotalBlocks()
        : Math.min(nFinishBlock, getApeInfoTotalBlocks());
