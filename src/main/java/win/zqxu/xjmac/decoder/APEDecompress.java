...
package win.zqxu.xjmac.decoder;

import java.io.IOException;

import win.zqxu.xjmac.info.*;
import win.zqxu.xjmac.prediction.IPredictorDecompress;
import win.zqxu.xjmac.prediction.PredictorDecompress3950toCurrent;
import win.zqxu.xjmac.prediction.PredictorDecompressNormal3930to3950;
...
public class APEDecompress extends IAPEDecompress {
  ...
  public APEDecompress(APEInfo pAPEInfo, int nStartBlock, int nFinishBlock) {
    ...
    m_nStartBlock = (nStartBlock < 0) ? 0
        : Math.min(nStartBlock, m_spAPEInfo.getApeInfoTotalBlocks());
    m_nFinishBlock = (nFinishBlock < 0) ? m_spAPEInfo.getApeInfoTotalBlocks()
        : Math.min(nFinishBlock, m_spAPEInfo.getApeInfoTotalBlocks());
  ...
  public int getApeInfoDecompressAverageBitrate() throws IOException {
    if (m_bIsRanged || !m_spAPEInfo.getApeInfoIoSource().isLocal()) {
      ...
      int nTotalBytes = (m_spAPEInfo.getApeInfoFrameBytes(nStartFrame)
          * (m_nStartBlock % nBlocksPerFrame)) / nBlocksPerFrame;
      if (nFinishFrame != nStartFrame)
        nTotalBytes += (m_spAPEInfo.getApeInfoFrameBytes(nFinishFrame)
            * (m_nFinishBlock % nBlocksPerFrame)) / nBlocksPerFrame;

      // get the number of bytes in between
      int nTotalFrames = m_spAPEInfo.getApeInfoTotalFrames();
      for (int nFrame = nStartFrame + 1; (nFrame < nFinishFrame)
          && (nFrame < nTotalFrames); nFrame++)
        nTotalBytes += m_spAPEInfo.getApeInfoFrameBytes(nFrame);

      // figure the bitrate
      int nTotalMS = (int) (((m_nFinishBlock - m_nStartBlock) * 1000L)
          / m_spAPEInfo.getApeInfoSampleRate());
      if (nTotalMS != 0)
  ...
  public byte[] getApeInfoWavHeaderData(int nMaxBytes) {
    if (m_bIsRanged) {
      if (WaveHeader.WAVE_HEADER_BYTES > nMaxBytes)
        ...
        WaveHeader.FillWaveHeader(WAVHeader,
            (m_nFinishBlock - m_nStartBlock) * m_spAPEInfo.getApeInfoBlockAlign(), wfeFormat, 0);
  ...
  public void SeekToFrame(int nFrameIndex) throws IOException {
    int nSeekRemainder = (m_spAPEInfo.getApeInfoSeekByte(nFrameIndex)
        - m_spAPEInfo.getApeInfoSeekByte(0)) % 4;
    m_spUnBitArray.FillAndResetBitArray(nFrameIndex == m_nRealFrame ? -1
        : m_spAPEInfo.getApeInfoSeekByte(nFrameIndex) - nSeekRemainder, nSeekRemainder * 8);
    ...
    try {
      if (m_wfeInput.nChannels == 2) {
        if ((m_nSpecialCodes & SpecialFrame.SPECIAL_FRAME_LEFT_SILENCE) > 0 &&
          ...
          for (nBlocksProcessed = 0; nBlocksProcessed < nBlocks; nBlocksProcessed++) {
            int X = m_spNewPredictorX
                .DecompressValue(m_spUnBitArray.DecodeValueRange(m_BitArrayStateX));
          ...
          if (m_spAPEInfo.getApeInfoFileVersion() >= 3950) {
            for (nBlocksProcessed = 0; nBlocksProcessed < nBlocks; nBlocksProcessed++) {
              ...
              m_Prepare.unprepare(X, Y, m_wfeInput, m_cbFrameBuffer.GetDirectWritePointer(),
                  m_nCRC);
              m_cbFrameBuffer.UpdateAfterDirectWrite(m_nBlockAlign);
            }
          } else {
            for (nBlocksProcessed = 0; nBlocksProcessed < nBlocks; nBlocksProcessed++) {
              int X = m_spNewPredictorX
                  .DecompressValue(m_spUnBitArray.DecodeValueRange(m_BitArrayStateX));
              int Y = m_spNewPredictorY
                  .DecompressValue(m_spUnBitArray.DecodeValueRange(m_BitArrayStateY));
  ...
  protected void StartFrame() throws IOException {
    ...
    // get any 'special' codes if the file uses them (for silence, FALSE stereo,
    // etc.)
    m_nSpecialCodes = 0;
    if (m_spAPEInfo.getApeInfoFileVersion() > 3820) {
      if ((m_nStoredCRC & 0x80000000L) > 0) {
        m_nSpecialCodes = (int) m_spUnBitArray
            .DecodeValue(DecodeValueMethod.DECODE_VALUE_METHOD_UNSIGNED_INT);
      }
  ...
  protected void InitializeDecompressor() throws IOException {
    ...
    m_cbFrameBuffer.CreateBuffer(
        (this.getApeInfoBlocksPerFrame() + DECODE_BLOCK_SIZE) * m_nBlockAlign, m_nBlockAlign * 64);

    // create decoding components
    m_spUnBitArray = UnBitArrayBase.CreateUnBitArray(this, m_spAPEInfo.getApeInfoFileVersion());

    if (m_spAPEInfo.getApeInfoFileVersion() >= 3950) {
      m_spNewPredictorX = new PredictorDecompress3950toCurrent(
          m_spAPEInfo.getApeInfoCompressionLevel(), m_spAPEInfo.getApeInfoFileVersion());
      m_spNewPredictorY = new PredictorDecompress3950toCurrent(
          m_spAPEInfo.getApeInfoCompressionLevel(), m_spAPEInfo.getApeInfoFileVersion());
    } else {
      m_spNewPredictorX = new PredictorDecompressNormal3930to3950(
          m_spAPEInfo.getApeInfoCompressionLevel(), m_spAPEInfo.getApeInfoFileVersion());
      m_spNewPredictorY = new PredictorDecompressNormal3930to3950(
          m_spAPEInfo.getApeInfoCompressionLevel(), m_spAPEInfo.getApeInfoFileVersion());
    }
