...
public class UnBitArrayBase {
  ...
  private final static long POWERS_OF_TWO_MINUS_ONE[] = {0, 1, 3, 7, 15, 31, 63, 127, 255, 511,
  ...
  public void FillBitArray() throws IOException {
    ...
    System.arraycopy(al = m_pBitArray, j = (int) nBitArrayIndex, al, 0,
        (int) (al.length - nBitArrayIndex));
  ...
  protected long DecodeValueXBits(long nBits) throws IOException {
    ...
    if (nLeftBits >= nBits)
      return ((long) (m_pBitArray[(int) nBitArrayIndex]
          & (POWERS_OF_TWO_MINUS_ONE[(int) nLeftBits]))) >> (nLeftBits - nBits);

    // must get the "split" value from left and right
    long nRightBits = nBits - nLeftBits;

    long nLeftValue = ((long) (m_pBitArray[(int) nBitArrayIndex]
        & POWERS_OF_TWO_MINUS_ONE[(int) nLeftBits])) << nRightBits;
