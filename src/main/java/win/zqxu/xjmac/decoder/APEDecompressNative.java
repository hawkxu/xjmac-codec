...
import win.zqxu.xjmac.info.APEInfo;
...
public class APEDecompressNative extends APEDecompress {
  ...
  public APEDecompressNative(APEInfo pAPEInfo, int nStartBlock, int nFinishBlock) {
    ...
    ID = registerDecoder(this.getApeInfoIoSource(), this.getApeInfoFileVersion(),
        this.getApeInfoCompressionLevel(), nStartBlock, nFinishBlock, this.getApeInfoTotalBlocks(),
        this.getApeInfoBlockAlign(), this.getApeInfoBlocksPerFrame(), this.getApeInfoSampleRate(),
        this.getApeInfoBitsPerSample(), this.getApeInfoChannels());
    if (ID < 0)
  ...
  private native int registerDecoder(File io, int nVersion, int nCompressionLevel, int nStartBlock,
      int nFinishBlock, int nTotalBlocks, int nBlockAlign, int nBlocksPerFrame, int nSampleRate,
