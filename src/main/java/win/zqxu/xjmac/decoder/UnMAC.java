...
public class UnMAC {
  ...
  public void SeekToFrame(int FrameIndex) throws IOException {
    if (m_pAPEDecompress.getApeInfoFileVersion() > 3800) {
      if ((m_LastDecodedFrameIndex == -1) || ((FrameIndex - 1) != m_LastDecodedFrameIndex)) {
        int SeekRemainder = (m_pAPEDecompress.getApeInfoSeekByte(FrameIndex)
            - m_pAPEDecompress.getApeInfoSeekByte(0)) % 4;
        m_pAPEDecompressCore.GetUnBitArrray().FillAndResetBitArray(
            m_nRealFrame == FrameIndex ? -1
                : m_pAPEDecompress.getApeInfoSeekByte(FrameIndex) - SeekRemainder,
            SeekRemainder * 8);
      ...
      if ((m_LastDecodedFrameIndex == -1) || ((FrameIndex - 1) != m_LastDecodedFrameIndex)) {
        m_pAPEDecompressCore.GetUnBitArrray().FillAndResetBitArray(
            m_nRealFrame == FrameIndex ? -1 : m_pAPEDecompress.getApeInfoSeekByte(FrameIndex),
            m_pAPEDecompress.getApeInfoSeekBit(FrameIndex));
        m_nRealFrame = FrameIndex;
      }
    }
  }
  ...
  private int DecompressFrameOld(ByteBuffer pOutputData, int FrameIndex) throws IOException {
    ...
    int nBlocks = 0;
    nBlocks = ((FrameIndex + 1) >= m_pAPEDecompress.getApeInfoTotalFrames())
        ? m_pAPEDecompress.getApeInfoFinalFrameBlocks()
        : m_pAPEDecompress.getApeInfoBlocksPerFrame();
    if (nBlocks == 0)
      throw new JMACException("Invalid Frame Index"); // nothing to do (file
                                                      // must be zero length)
    ...
    if ((m_pAPEDecompress.getApeInfoFormatFlags() & APEHeader.MAC_FORMAT_FLAG_CRC) <= 0) {
      nStoredCRC = m_pAPEDecompressCore.GetUnBitArrray()
          .DecodeValue(DecodeValueMethod.DECODE_VALUE_METHOD_UNSIGNED_RICE, 30);
      if (nStoredCRC == 0)
        nSpecialCodes = SpecialFrame.SPECIAL_FRAME_LEFT_SILENCE
            | SpecialFrame.SPECIAL_FRAME_RIGHT_SILENCE;
    } else {
      nStoredCRC = m_pAPEDecompressCore.GetUnBitArrray()
          .DecodeValue(DecodeValueMethod.DECODE_VALUE_METHOD_UNSIGNED_INT);

      // get any 'special' codes if the file uses them (for silence, FALSE
      // stereo, etc.)
      nSpecialCodes = 0;
      if (m_pAPEDecompress.getApeInfoFileVersion() > 3820) {
        if ((nStoredCRC & 0x80000000) > 0)
          nSpecialCodes = m_pAPEDecompressCore.GetUnBitArrray()
              .DecodeValue(DecodeValueMethod.DECODE_VALUE_METHOD_UNSIGNED_INT);
    ...
    if (m_pAPEDecompress.getApeInfoChannels() == 2) {
      ...
      m_pPrepare.unprepareOld(m_pAPEDecompressCore.m_pDataX, m_pAPEDecompressCore.m_pDataY, nBlocks,
          m_wfeInput,
    ...
    // check the CRC
    if ((m_pAPEDecompress.getApeInfoFormatFlags() & APEHeader.MAC_FORMAT_FLAG_CRC) <= 0) {
      long nChecksum = CalculateOldChecksum(m_pAPEDecompressCore.m_pDataX,
          m_pAPEDecompressCore.m_pDataY, m_pAPEDecompress.getApeInfoChannels(), nBlocks);
