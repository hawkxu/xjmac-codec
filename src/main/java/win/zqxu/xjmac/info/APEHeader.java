...
import win.zqxu.xjmac.tools.ByteArrayReader;
...
public class APEHeader {
  ...
  public final static int MAC_FORMAT_FLAG_CRC = 2; // uses the new CRC32 error
                                                   // detection
  public final static int MAC_FORMAT_FLAG_HAS_PEAK_LEVEL = 4; // unsigned
                                                              // __int32
                                                              // Peak_Level
                                                              // after the
                                                              // header
  public final static int MAC_FORMAT_FLAG_24_BIT = 8; // is 24-bit
  public final static int MAC_FORMAT_FLAG_HAS_SEEK_ELEMENTS = 16; // has the
                                                                  // number of
                                                                  // seek
                                                                  // elements
                                                                  // after the
                                                                  // peak level
  public final static int MAC_FORMAT_FLAG_CREATE_WAV_HEADER = 32; // create the
                                                                  // wave header
                                                                  // on
                                                                  // decompression
                                                                  // (not
  ...
  protected void AnalyzeCurrent(APEFileInfo m_APEFileInfo) throws IOException {
    ...
    if ((m_APEFileInfo.spAPEDescriptor.nDescriptorBytes - APEDescriptor.APE_DESCRIPTOR_BYTES) > 0)
      m_pIO.skipBytes((int) (m_APEFileInfo.spAPEDescriptor.nDescriptorBytes
          - APEDescriptor.APE_DESCRIPTOR_BYTES));
    ...
    if ((m_APEFileInfo.spAPEDescriptor.nHeaderBytes - APEHeaderNew.APE_HEADER_BYTES) > 0)
      m_pIO.skipBytes(
          (int) (m_APEFileInfo.spAPEDescriptor.nHeaderBytes - APEHeaderNew.APE_HEADER_BYTES));
    ...
    m_APEFileInfo.nTotalBlocks = (int) ((APEHeader.nTotalFrames == 0) ? 0
        : ((APEHeader.nTotalFrames - 1) * m_APEFileInfo.nBlocksPerFrame)
            + APEHeader.nFinalFrameBlocks);
    m_APEFileInfo.nWAVHeaderBytes = (int) ((APEHeader.nFormatFlags
        & MAC_FORMAT_FLAG_CREATE_WAV_HEADER) > 0 ? WaveHeader.WAVE_HEADER_BYTES
            : m_APEFileInfo.spAPEDescriptor.nHeaderDataBytes);
    ...
    m_APEFileInfo.nWAVTotalBytes = m_APEFileInfo.nWAVDataBytes + m_APEFileInfo.nWAVHeaderBytes
        + m_APEFileInfo.nWAVTerminatingBytes;
    m_APEFileInfo.nAPETotalBytes = m_pIO.isLocal() ? (int) m_pIO.length() : -1;
    m_APEFileInfo.nLengthMS = (int) ((m_APEFileInfo.nTotalBlocks * 1000L)
        / m_APEFileInfo.nSampleRate);
    m_APEFileInfo.nAverageBitrate = (m_APEFileInfo.nLengthMS <= 0) ? 0
        : (int) ((m_APEFileInfo.nAPETotalBytes * 8L) / m_APEFileInfo.nLengthMS);
    m_APEFileInfo.nDecompressedBitrate = (m_APEFileInfo.nBlockAlign * m_APEFileInfo.nSampleRate * 8)
        / 1000;
    m_APEFileInfo.nSeekTableElements = (int) (m_APEFileInfo.spAPEDescriptor.nSeekTableBytes / 4);
    m_APEFileInfo.nPeakLevel = -1;

    // get the seek tables (really no reason to get the whole thing if there's
    // extra)
  ...
  protected void AnalyzeOld(APEFileInfo m_APEFileInfo) throws IOException {
    ...
    m_APEFileInfo.nBlocksPerFrame = ((header.nVersion >= 3900) || ((header.nVersion >= 3800)
        && (header.nCompressionLevel == CompressionLevel.COMPRESSION_LEVEL_EXTRA_HIGH))) ? 73728
            : 9216;
    ...
    m_APEFileInfo.nBitsPerSample = (m_APEFileInfo.nFormatFlags & MAC_FORMAT_FLAG_8_BIT) > 0 ? 8
        : ((m_APEFileInfo.nFormatFlags & MAC_FORMAT_FLAG_24_BIT) > 0 ? 24 : 16);
    m_APEFileInfo.nBytesPerSample = m_APEFileInfo.nBitsPerSample / 8;
    m_APEFileInfo.nBlockAlign = m_APEFileInfo.nBytesPerSample * m_APEFileInfo.nChannels;
    m_APEFileInfo.nTotalBlocks = (int) ((header.nTotalFrames == 0) ? 0
        : ((header.nTotalFrames - 1) * m_APEFileInfo.nBlocksPerFrame) + header.nFinalFrameBlocks);
    m_APEFileInfo.nWAVHeaderBytes = (int) ((header.nFormatFlags
        & MAC_FORMAT_FLAG_CREATE_WAV_HEADER) > 0 ? WaveHeader.WAVE_HEADER_BYTES
            : header.nHeaderBytes);
    ...
    m_APEFileInfo.nLengthMS = (int) ((m_APEFileInfo.nTotalBlocks * 1000L)
        / m_APEFileInfo.nSampleRate);
    m_APEFileInfo.nAverageBitrate = (int) ((m_APEFileInfo.nLengthMS <= 0) ? 0
        : ((m_APEFileInfo.nAPETotalBytes * 8L) / m_APEFileInfo.nLengthMS));
    m_APEFileInfo.nDecompressedBitrate = (m_APEFileInfo.nBlockAlign * m_APEFileInfo.nSampleRate * 8)
        / 1000;
  ...
  protected int FindDescriptor(boolean bSeek) throws IOException {
    ...
    if (m_pIO.isLocal()) {
      ...
      if (tag.equals("ID3")) {
        ...
        // error check
        if ((byte5 & 64) > 0) {
          // this ID3v2 length calculator algorithm can't cope with extended
          // headers
          // we should be ok though, because the scan for the MAC header below
          // should
          // really do the trick
        }
    ...
    // Also, lets suppose that MAC header placed in beginning of file in case of
    // external source of file
    if (m_pIO.isLocal()) {
      int nScanBytes = 0;
