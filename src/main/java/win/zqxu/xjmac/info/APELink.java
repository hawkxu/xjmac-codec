  ...
  protected void ParseData(byte[] pData, String pFilename) {
    ...
    if (pData != null) {
      ...
      if (pHeader >= 0 && pImageFile >= 0 && pStartBlock >= 0 && pFinishBlock >= 0) {
        ...
        try {
          m_nStartBlock = Integer.parseInt(data.substring(
              pStartBlock + APE_LINK_START_BLOCK_TAG.length(), ii >= 0 ? ii : data.length()));
        } catch (Exception e) {
          m_nStartBlock = -1;
        }
        ...
        try {
          m_nFinishBlock = Integer.parseInt(data.substring(
              pFinishBlock + APE_LINK_FINISH_BLOCK_TAG.length(), ii >= 0 ? ii : data.length()));
        } catch (Exception e) {
          m_nFinishBlock = -1;
        }
        ...
        i2 = data.indexOf('\n', pImageFile);
        ii = i1 > 0 && i2 > 0 ? Math.min(i1, i2) : Math.max(i1, i2);
        String cImageFile = data.substring(pImageFile + APE_LINK_IMAGE_FILE_TAG.length(),
            ii >= 0 ? ii : data.length());
