...
import win.zqxu.xjmac.tools.RandomAccessFile;
...
public class APEInfo {
  ...
  public APEInfo(File pIO, APETag pTag) throws IOException {
    ...
    if (pTag == null) {
      ...
      // since a single I/O object is shared, we can't tag and read at the same
      // time (i.e. in multiple threads)
  ...
  public int getApeInfoSeekByte(int nFrame) {
    return (nFrame < 0 || nFrame >= m_APEFileInfo.nTotalFrames) ? 0
        : m_APEFileInfo.spSeekByteTable[nFrame] + m_APEFileInfo.nJunkHeaderBytes;
  }
  ...
  public int getApeInfoFrameBytes(int nFrame) throws IOException {
    if ((nFrame < 0) || (nFrame >= m_APEFileInfo.nTotalFrames))
      ...
      if (nFrame != (m_APEFileInfo.nTotalFrames - 1))
        ...
        if (m_spIO.isLocal())
          return (int) m_spIO.length() - m_spAPETag.GetTagBytes()
              - m_APEFileInfo.nWAVTerminatingBytes - getApeInfoSeekByte(nFrame);
        else if (nFrame > 0)
  ...
  public WaveFormat getApeInfoWaveFormatEx() {
    ...
    WaveFormat.FillWaveFormatEx(pWaveFormatEx, m_APEFileInfo.nSampleRate,
        m_APEFileInfo.nBitsPerSample, m_APEFileInfo.nChannels);
  ...
  public byte[] getApeInfoWavHeaderData(int nMaxBytes) {
    if ((m_APEFileInfo.nFormatFlags & APEHeader.MAC_FORMAT_FLAG_CREATE_WAV_HEADER) > 0) {
      if (WaveHeader.WAVE_HEADER_BYTES > nMaxBytes)
        ...
        WaveHeader.FillWaveHeader(WAVHeader, m_APEFileInfo.nWAVDataBytes, wfeFormat,
      ...
      if (m_APEFileInfo.nWAVHeaderBytes > nMaxBytes)
        ...
        System.arraycopy(m_APEFileInfo.spWaveHeaderData, 0, pBuffer, 0,
            m_APEFileInfo.nWAVHeaderBytes);
        return pBuffer;
      }
    }
  }
  ...
  public byte[] getApeInfoWavTerminatingData(int nMaxBytes) throws IOException {
    if (m_APEFileInfo.nWAVTerminatingBytes > nMaxBytes)
      ...
      if (m_APEFileInfo.nWAVTerminatingBytes > 0) {
        ...
        // check for a tag
        m_spIO.seek(
            m_spIO.length() - (m_spAPETag.GetTagBytes() + m_APEFileInfo.nWAVTerminatingBytes));
