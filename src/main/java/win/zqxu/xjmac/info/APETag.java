...
public class APETag implements Comparator {
  ...
  // create an APE tag
  // bAnalyze determines whether it will analyze immediately or on the first
  // request
  // be careful with multiple threads / file pointer movement if you don't
  // analyze immediately
  ...
  // save the tag to the I/O source (bUseOldID3 forces it to save as an ID3v1.1
  // tag instead of an APE tag)
  ...
  // removes any tags from the file (bUpdate determines whether is should
  // re-analyze after removing the tag)
  ...
  public void SetFieldBinary(String pFieldName, byte[] pFieldValue, int nFieldFlags)
      throws IOException {
    if (!m_bAnalyzed)
      Analyze();
    ...
    // check to see if we're trying to remove the field (by setting it to NULL
    // or an empty string)
  ...
  // gets the value of a field (returns -1 and an empty buffer if the field
  // doesn't exist)
  ...
  // fills in an ID3_TAG using the current fields (useful for quickly converting
  // the tag)
  ...
  private int GetTagFieldIndex(String pFieldName) throws IOException {
    ...
    for (int z = 0; z < m_aryFields.size(); z++) {
      if (pFieldName.toLowerCase()
          .equals(((APETagField) m_aryFields.get(z)).GetFieldName().toLowerCase()))
        return z;
    }
