...
import win.zqxu.xjmac.prediction.IPredictorCompress;
import win.zqxu.xjmac.prediction.PredictorCompressNormal;
import win.zqxu.xjmac.tools.*;

/**
 * Author: Dmitry Vaguine Date: 08.05.2004 Time: 11:18:47
 */
public class APECompressCore {
  public APECompressCore(File pIO, WaveFormat pwfeInput, int nMaxFrameBlocks,
    ...
    if (m_wfeInput.nChannels == 2) {
      ...
      if (bEncodeX && bEncodeY) {
        ...
        for (int z = 0; z < nInputBlocks; z++) {
          m_spBitArray.EncodeValue(m_spPredictorY.CompressValue(m_spDataY[z], nLastX),
              m_BitArrayStateY);
          m_spBitArray.EncodeValue(m_spPredictorX.CompressValue(m_spDataX[z], m_spDataY[z]),
              m_BitArrayStateX);
  ...
  private void Prepare(ByteArrayReader pInputData, int nInputBytes, IntegerPointer pSpecialCodes)
      throws IOException {
    // variable declares
